defmodule MapSets do
  ...
  koan "Are these maps twins?" do
    ...
    assert MapSet.equal?(@set, MapSet.new([5,3,2,4,1])) == true
  end
  ...
  koan "I want only the common values in both sets" do
    ...
    assert MapSet.equal?(intersection_set, MapSet.new([5]))
  end
  ...
  koan "Unify my sets" do
    ...
    assert MapSet.equal?(new_set, MapSet.new([1,2,3,4,5,6,7]))
  end

  koan "I want my set in a list" do
    assert MapSet.to_list(@set) == [1, 2, 3, 4, 5]
    # mmmm so set seems ordered in a certain way...
  end
end
