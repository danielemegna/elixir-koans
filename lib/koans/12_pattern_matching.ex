defmodule PatternMatching do
  ...
  koan "Maps support partial pattern matching" do
    map = %{type: "car", year: 2016, make: "Honda", color: "black"}
    %{make: make, type: type} = map

    assert make == "Honda"
    assert type == "car"
    assert map.make == "Honda"
    assert map.type == "car"
  end
  ...
  koan "So does the keyword lists" do
    ...
    [type | [_year | [tuple]]] = kw_list
    assert tuple == {:make, "Honda"}
    assert type == {:type, "car"}
  end

  koan "The pattern can make assertions about what it expects" do
    assert match?([1, _second, _third], [1, "any", :value])
  end
  ...
  koan "Errors are shaped differently than successful results" do
    ...
    result = case Map.fetch(dog, :name) do
      {:ok, value} -> value
      :error -> "not present"
    end
  ...
  koan "Structs will even match with a regular map" do
    %{kind: kind} = %Animal{kind: "dog", name: "Max"}
    assert kind == "dog"
  end
